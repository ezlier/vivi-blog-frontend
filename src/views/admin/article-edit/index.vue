<template>
  <div class="page admin-article-edit">
    <div class="editor-header">
      ...
      <el-button class="settings-button" @click="showSettings = true">设置</el-button>
    </div>

    <el-form v-loading="loading" class="article-form">
      <MdEditor previewTheme="arknights" v-model="form.content" style="height: 70dvh" />
    </el-form>
    ...
    <el-drawer v-model="showSettings" direction="rtl" size="min(90vw, 360px)" :with-header="false" :show-close="false"
      :close-on-click-modal="true" modal-class="article-settings-modal" class="article-settings-drawer">
      <div class="settings-panel">
        <div class="settings-header">
          <button type="button" class="settings-close" aria-label="关闭设置" @click="showSettings = false">&times;</button>
          <span>文章设置</span>
        </div>

        <el-form label-position="top" class="settings-form">
          <el-form-item label="标签">
            <el-select v-model="form.tagNames" multiple filterable allow-create default-first-option
              :reserve-keyword="false" placeholder="输入或选择标签" style="width: 100%">
          ...
          <el-form-item v-if="isEdit" label="创建时间">
            <el-date-picker v-model="form.created_at" type="datetime" value-format="YYYY-MM-DDTHH:mm:ss"
              placeholder="选择创建时间" style="width: 100%" />
          </el-form-item>
        </el-form>
      </div>
    </el-drawer>
...
import { createArticle, updateArticle, getArticle as getAdminArticle } from '@/api/article'
...
onMounted(async () => {
  try {
    ...
    availableTags.value = (tags?.items || []).map((tag: { name: string }) => tag.name)
  } catch {
    availableTags.value = []
  }
  ...
  if (isEdit.value) {
    ...
    try {
      ...
      form.value.tagNames = Array.isArray(article.tags)
        ? article.tags.map((tag: string | { name: string }) => typeof tag === 'string' ? tag : tag.name)
        : []
      form.value.created_at = article.created_at ? article.created_at.slice(0, 19) : ''
